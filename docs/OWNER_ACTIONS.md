...
9. **Billing model:** Free, Basic, Standard, Pro (`premium`) and Elite are active in application code. Basic is 49 SEK/month for 3 months then 69; Standard 79 then 119; Pro 159 then 179; Elite 399 with no launch offer. A customer may redeem only one StockBox launch offer total. Eligible affiliate referrals receive 10% off regular pricing when no better launch promotion applies. Affiliate commission is configured individually in Admin, and payouts run monthly with a 100 SEK minimum.
...
4. **Post-v2.7 report QA - BLOCKER.** Run real Summary/Numbers/Deep analyses across a representative issuer set including US mega-cap, Swedish/global stock, financial, REIT, cyclical/commodity, loss-making growth, cross-currency, stale/partial fundamentals and unsupported security cases. Review score, neutral research view, confidence, provenance, DCF gating and missing-data behavior. Do not tune the engine toward preferred company outcomes.
...
6. **Production abuse/telemetry QA - BLOCKER.** Validate deployed rate limiting/WAF behavior across instances, inspect PostHog payloads for allowlisted non-sensitive data only, and verify Strong research-view admin-alert delivery/retry/dedup against the configured email provider.

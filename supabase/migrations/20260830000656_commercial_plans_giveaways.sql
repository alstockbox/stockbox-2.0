...
insert into public.plans (key, name, monthly_price_sek, entitlements, active)
values
  ('free', 'Free', null, '{"monthlyAnalyses":3,"deepAnalyses":1,"watchlistItems":5,"batchRows":0,"portfolios":1,"aiAssistant":false,"hourlyAlerts":false}'::jsonb, true),
  ('basic', 'Basic', 69, '{"monthlyAnalyses":10,"deepAnalyses":3,"watchlistItems":20,"batchRows":10,"portfolios":2,"aiAssistant":false,"hourlyAlerts":false}'::jsonb, true),
  ('standard', 'Standard', 119, '{"monthlyAnalyses":35,"deepAnalyses":12,"watchlistItems":75,"batchRows":25,"portfolios":5,"aiAssistant":true,"hourlyAlerts":true}'::jsonb, true),
  ('premium', 'Pro', 179, '{"monthlyAnalyses":90,"deepAnalyses":35,"watchlistItems":250,"batchRows":50,"portfolios":15,"aiAssistant":true,"hourlyAlerts":true}'::jsonb, true),
  ('elite', 'Elite', 399, '{"monthlyAnalyses":350,"deepAnalyses":150,"watchlistItems":1000,"batchRows":50,"portfolios":50,"aiAssistant":true,"hourlyAlerts":true}'::jsonb, true)
...
create or replace function private.stockbox_plan_rank(p_plan_key text)
returns integer
language sql
immutable
as $$
...
$$;
create table public.affiliate_giveaway_campaigns (
  id uuid primary key default gen_random_uuid(),
  affiliate_id uuid not null references public.affiliates(id) on delete restrict,
  created_by uuid not null references public.profiles(id) on delete restrict,
  label text not null check (char_length(label) between 1 and 120),
  plan_key text not null check (plan_key in ('basic','standard','premium','elite')),
...
create table public.affiliate_giveaway_codes (
  id uuid primary key default gen_random_uuid(),
  campaign_id uuid not null references public.affiliate_giveaway_campaigns(id) on delete restrict,
...
create table public.promotional_access_grants (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references public.profiles(id) on delete cascade,
  campaign_id uuid not null references public.affiliate_giveaway_campaigns(id) on delete restrict,
  code_id uuid not null unique references public.affiliate_giveaway_codes(id) on delete restrict,
  plan_key text not null check (plan_key in ('basic','standard','premium','elite')),
...
create index affiliate_giveaway_campaigns_affiliate_idx
  on public.affiliate_giveaway_campaigns (affiliate_id, created_at desc);
create index affiliate_giveaway_codes_campaign_status_idx
  on public.affiliate_giveaway_codes (campaign_id, status);
create index promotional_access_grants_user_active_idx
  on public.promotional_access_grants (user_id, status, starts_at, ends_at);

alter table public.affiliate_giveaway_campaigns enable row level security;
alter table public.affiliate_giveaway_codes enable row level security;
alter table public.promotional_access_grants enable row level security;

revoke all on public.affiliate_giveaway_campaigns from public, anon, authenticated;
revoke all on public.affiliate_giveaway_codes from public, anon, authenticated;
revoke all on public.promotional_access_grants from public, anon, authenticated;
grant select, insert, update on public.affiliate_giveaway_campaigns to service_role;
grant select, insert, update on public.affiliate_giveaway_codes to service_role;
grant select, insert, update on public.promotional_access_grants to service_role;
...
create or replace function public.revoke_affiliate_giveaway_campaign(
  p_actor_id uuid,
  p_campaign_id uuid
)
returns jsonb
...
begin
  ...
  perform 1 from public.affiliate_giveaway_campaigns
  where id = p_campaign_id for update;
  if not found then raise exception 'giveaway campaign not found'; end if;

  update public.affiliate_giveaway_campaigns
  ...
  return jsonb_build_object('ok', true, 'campaignId', p_campaign_id, 'revokedCodes', v_changed);
end;
$$;

revoke all on function public.revoke_affiliate_giveaway_campaign(uuid, uuid)
  from public, anon, authenticated;
grant execute on function public.revoke_affiliate_giveaway_campaign(uuid, uuid)
  to service_role;
...
create or replace function private.stockbox_effective_plan(p_user_id uuid)
returns text
language plpgsql
security definer
set search_path = public, private
...
declare
  ...
  v_promo_plan text := 'free';
begin
  select s.plan_key into v_paid_plan
  from public.subscriptions s
  join public.plans p on p.key = s.plan_key and p.active = true
  where s.user_id = p_user_id
    and s.status in ('active', 'trialing')
  limit 1;
  v_paid_plan := coalesce(v_paid_plan, 'free');

  select g.plan_key into v_promo_plan
  from public.promotional_access_grants g
  where g.user_id = p_user_id
    and g.status = 'active'
    and g.starts_at <= now()
    and g.ends_at > now()
  order by private.stockbox_plan_rank(g.plan_key) desc, g.ends_at desc
  limit 1;
  v_promo_plan := coalesce(v_promo_plan, 'free');

  if private.stockbox_plan_rank(v_promo_plan) > private.stockbox_plan_rank(v_paid_plan) then
    return v_promo_plan;
  end if;
  return v_paid_plan;
end;
...
revoke all on function private.stockbox_effective_plan(uuid)
  from public, anon, authenticated;

...
const discountedQuality: MispricingInput = {
  ...
  trends: {
    revenueGrowthCurrent: 0.18, revenueGrowthPrior: 0.14,
    epsGrowthCurrent: 0.22, epsGrowthPrior: 0.16,
    fcfMarginCurrent: 0.19, fcfMarginPrior: 0.17,
    operatingMarginCurrent: 0.23, operatingMarginPrior: 0.21,
    cashConversion: 1.05, debtToEquity: 0.35, interestCoverage: 18, shareGrowth: -0.01,
  },
  revisionNetLastMonth: 4,
  redFlags: [], sourceConflictSeverity: "none", dataStatus: "current", dataAsOf: "2026-08-31",
};
...
describe("computeMispricingAssessment", () => {
  ...
  it("penalizes a superficially cheap but deteriorating value trap", () => {
    const result = computeMispricingAssessment({ ...discountedQuality,
      trends: { revenueGrowthCurrent: -0.12, revenueGrowthPrior: 0.12, epsGrowthCurrent: -0.3, epsGrowthPrior: 0.1,
        fcfMarginCurrent: 0.02, fcfMarginPrior: 0.14, operatingMarginCurrent: 0.04, operatingMarginPrior: 0.16,
        cashConversion: 0.35, debtToEquity: 2.8, interestCoverage: 1.4, shareGrowth: 0.18 },
      revisionNetLastMonth: -8, redFlags: [{ severity: "high", title: "Funding stress" }],
    });
    expect(result.score).not.toBeNull();
  ...
  it("does not score unavailable DCF as zero", () => {
    const result = computeMispricingAssessment({ ...discountedQuality, dcf: { suitable: false, bear: null, base: null, bull: null, confidence: 0 } });
    expect(result.pillars.find((pillar) => pillar.id === "intrinsic_value")?.score).toBeNull();
    expect(result.score).not.toBeNull();
  ...
  it("reduces confidence for stale and conflicted data without fabricating a bearish score", () => {
    ...
    const uncertain = computeMispricingAssessment({ ...discountedQuality, sourceConflictSeverity: "high", dataStatus: "stale" });

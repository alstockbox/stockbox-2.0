...
const earlyInflection: InflectionInput = {
  fundamentals: { revenueGrowthCurrent: 0.24, revenueGrowthPrior: 0.11, epsGrowthCurrent: 0.31, epsGrowthPrior: 0.09, fcfMarginCurrent: 0.16, fcfMarginPrior: 0.1, operatingMarginCurrent: 0.19, operatingMarginPrior: 0.14, roicCurrent: 0.18, roicPrior: 0.13 },
  expectations: { revisionNetLastWeek: 3, revisionNetLastMonth: 7, nextYearRevenueGrowth: 0.19, nextYearEpsGrowth: 0.25 },
  market: { oneMonth: 0.07, threeMonth: 0.16, sixMonth: 0.22, oneYear: 0.28, price: 92, yearHigh: 110, yearLow: 55 },
  funding: { financialHealthScore: 82, shareGrowth: 0.01, interestCoverage: 11, criticalRisk: false },
  research: { positiveCatalysts: 2, negativeCatalysts: 0 }, dataAsOf: "2026-08-31",
};
...
describe("computeInflectionAssessment", () => {
  ...
  it("does not call momentum alone a high-conviction inflection", () => {
    const result = computeInflectionAssessment({ fundamentals: null, expectations: null,
      market: { oneMonth: 0.18, threeMonth: 0.42, sixMonth: 0.55, oneYear: 0.7, price: 99, yearHigh: 100, yearLow: 40 },
      funding: { financialHealthScore: 62, shareGrowth: null, interestCoverage: null, criticalRisk: false }, research: null });
  ...
  it("keeps fundamental acceleration in building stage without market confirmation", () => {
    const result = computeInflectionAssessment({ ...earlyInflection, market: { oneMonth: -0.03, threeMonth: -0.07, sixMonth: -0.04, oneYear: 0.01, price: 65, yearHigh: 110, yearLow: 55 } });
  ...
  it("caps a fragile setup despite strong growth and momentum", () => {
    const result = computeInflectionAssessment({ ...earlyInflection, funding: { financialHealthScore: 22, shareGrowth: 0.2, interestCoverage: 0.9, criticalRisk: true } });
  ...
  it("penalizes dilution even when other signals are constructive", () => {
    ...
    const diluted = computeInflectionAssessment({ ...earlyInflection, funding: { ...earlyInflection.funding!, shareGrowth: 0.16 } });
  ...
  it("marks parabolic price action as extended instead of blindly increasing conviction", () => {
    const result = computeInflectionAssessment({ ...earlyInflection, market: { oneMonth: 0.38, threeMonth: 0.78, sixMonth: 1.1, oneYear: 1.45, price: 119, yearHigh: 120, yearLow: 44 } });

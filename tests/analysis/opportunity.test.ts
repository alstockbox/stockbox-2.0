...
describe("computeOpportunityAssessment", () => {
  ...
  it("weights inflection most heavily for short-term analysis", () => {
    ...
    expect(shortTerm.score).not.toBeNull();
    expect(longTerm.score).not.toBeNull();
  ...
  it("does not mutate canonical component values", () => {
    const input = { coreScore: 77, mispricingScore: 68, inflectionScore: 73, profile: "growth" as const };
    const before = structuredClone(input);
    computeOpportunityAssessment(input);

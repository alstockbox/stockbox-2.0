...
describe("Yahoo corporate-action history coverage", () => {
  ...
  it("does not merge maximum-history corporate actions from a different Yahoo symbol", async () => {
    const recent = chartPayload({
      symbol: "DIV",
      date: "2026-09-03",
      price: 70,
    ...
    const mismatchedHistory = chartPayload({
      symbol: "MSFT",
      date: "2026-09-03",
      price: 500,
      dividends: [{ date: "1990-03-15", amount: 9.99 }],
      splits: [{ date: "1996-05-13", numerator: 99, denominator: 1 }],
    });

    vi.spyOn(globalThis, "fetch").mockImplementation((input) => {
      const url = new URL(String(input));
      return Promise.resolve(json(url.searchParams.get("range") === "max" ? mismatchedHistory : recent));
    });

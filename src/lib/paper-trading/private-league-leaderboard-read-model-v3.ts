...
type PrivateLeagueCompetitionStatusV3 = "open" | "active" | "completed" | "cancelled";

type PrivateLeagueCompetitionTermsV3 = {
  id: string;
  kind: "challenge" | "private_league";
  status: PrivateLeagueCompetitionStatusV3;
  baseCurrency: string;
  startsAt: string;
  endsAt: string;
};

type PrivateLeagueCompetitionLoadResultV3 =
  | { ok: true; competition: PrivateLeagueCompetitionTermsV3 }
...
export type PaperPrivateLeagueLeaderboardPresentationStandingV3 = {
  rank: number;
...
export type PaperPrivateLeagueLeaderboardReadModelResultV3 =
  | {
      status: "VERIFIED";
      ...
      standings: PaperPrivateLeagueLeaderboardPresentationStandingV3[];
    }
  | {
      status: "UNAVAILABLE";
      reason:
...
export type PaperPrivateLeagueLeaderboardReadModelDependenciesV3 = {
  loadCompetition: (competitionId: string) => Promise<PrivateLeagueCompetitionLoadResultV3>;
...
function mapCompetition(row: JsonRow): PrivateLeagueCompetitionTermsV3 | null {
...
async function loadPrivateLeagueCompetitionTermsV3(competitionId: string): Promise<PrivateLeagueCompetitionLoadResultV3> {
...
async function loadLatestVerifiedPrivateLeagueCutoffV3(competitionId: string): Promise<VerifiedCutoffLoadResultV3> {
  if (!UUID_PATTERN.test(competitionId)) return { ok: false, error: "INVALID_INPUT" };
  const supabase = createAdminClient();
  if (!supabase) return { ok: false, error: "SUPABASE_ADMIN_NOT_CONFIGURED" };
...
const defaultDependencies: PaperPrivateLeagueLeaderboardReadModelDependenciesV3 = {
  loadCompetition: loadPrivateLeagueCompetitionTermsV3,
  loadVerifiedCutoff: loadLatestVerifiedPrivateLeagueCutoffV3,
...
function standingsAreComplete(input: {
  competition: PrivateLeagueCompetitionTermsV3;
  evaluationCutoff: string;
  viewerUserId: string;
  ...
  if (
    ...
    || input.result.competitionKind !== "private_league"
...
/**
 * Builds a presentation-safe private league leaderboard exclusively from the
 * latest persisted verified cutoff and exact-cutoff snapshots. Membership is
 * re-authorized by the standings repository through member scope. This path
 * never fetches quotes, recomputes performance, falls back to latest snapshots,
 * or exposes raw participant/account identifiers in the UI model.
 */
export async function loadPaperPrivateLeagueLeaderboardReadModelV3(
  input: { competitionId: string; viewerUserId: string },
  dependencies: PaperPrivateLeagueLeaderboardReadModelDependenciesV3 = defaultDependencies,
): Promise<PaperPrivateLeagueLeaderboardReadModelResultV3> {
  ...
  let competitionResult: PrivateLeagueCompetitionLoadResultV3;
  try {
  ...
  if (
    competition.kind !== "private_league"
  ...
  if (!pointerResult.ok || !pointerResult.pointer) {
    return { status: "UNAVAILABLE", reason: "NO_VERIFIED_CUTOFF" };
  }

  ...
  try {
    ...
    return {
      ...
      source: {
        ...
        freshness: "Monthly historical prices, adjusted prices and corporate actions, cached up to 30 minutes.",
        provider: PROVIDER_ID,
        capability: "market_data",
        dataAsOf: adjustedPriceHistory.at(-1)?.date ?? priceHistory.at(-1)?.date ?? dividendEvents.at(-1)?.date ?? null,
      },
    };
  } catch (error) {

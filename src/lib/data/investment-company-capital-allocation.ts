...
function hasCompleteBenchmark(point: InvestmentCompanyKeyRatioYear): boolean {
  return typeof point.benchmarkReturnSixrx === "number" && Number.isFinite(point.benchmarkReturnSixrx);
}

function hasValidReturnHistory(point: InvestmentCompanyKeyRatioYear): boolean {
  return Number.isFinite(point.portfolioReturn)
    && (point.portfolioReturn as number) > -1
...
function isMaterialPositiveDeployment(point: InvestmentCompanyKeyRatioYear): boolean {
  if (!hasCompleteFundingEvidence(point) || (point.netPurchasesSales as number) <= 0) return false;
  const navEstimate = (point.navPerShare as number) * (point.sharesOutstanding as number);
  if (!Number.isFinite(navEstimate) || navEstimate <= 0) return false;
  return (point.netPurchasesSales as number) / navEstimate >= MATERIAL_DEPLOYMENT_NAV_RATIO;
}
...
function portfolioOutcomeScore(relativePortfolioCagr: number): number {
  ...
  return clampScore(50 + (relativePortfolioCagr / FULL_RELATIVE_OUTPERFORMANCE_CAGR) * 50);
}
...
function fundingDisciplineScore(
  ...
  return clampScore(100 * (1 - averageIncrease / FULL_FUNDING_PENALTY_DEBT_RATIO_INCREASE));
}
...
export function deriveInvestmentCompanyCapitalAllocation(
  ...
  if (ordered.length < REQUIRED_SCORING_YEARS) return failure("insufficient_consecutive_history");

  const selected = ordered.slice(0, REQUIRED_SCORING_YEARS);
  const yearsUsed = selected.map((point) => point.year);
  const consecutive = new Set(yearsUsed).size === REQUIRED_SCORING_YEARS
    && yearsUsed.every((year, index) => index === 0 || year === yearsUsed[index - 1] - 1);
  if (!consecutive) return failure("insufficient_consecutive_history", yearsUsed);
  if (!selected.every(hasCompleteBenchmark)) return failure("incomplete_benchmark_history", yearsUsed);
  if (!selected.every(hasValidReturnHistory)) return failure("invalid_return_history", yearsUsed);
  if (!selected.every(hasCompleteFundingEvidence)) return failure("incomplete_funding_history", yearsUsed);
  ...
  if (deploymentPoints.length < MIN_MATERIAL_DEPLOYMENT_YEARS) {
    return failure("insufficient_material_deployment_history", yearsUsed, deploymentYears);
  }
  ...
  for (const current of selected) {
    ...
    const shareGrowth = (current.sharesOutstanding as number) / (previous.sharesOutstanding as number) - 1;
  ...
  for (const point of selected) {
    relativeGrowth *= (1 + (point.portfolioReturn as number)) / (1 + (point.benchmarkReturnSixrx as number));
  }
  const relativePortfolioCagr = relativeGrowth ** (1 / REQUIRED_SCORING_YEARS) - 1;
  if (!Number.isFinite(relativePortfolioCagr)) return failure("invalid_return_history", yearsUsed, deploymentYears);
  ...
  if (fundingScore === null) return failure("incomplete_funding_history", yearsUsed, deploymentYears);

  return {
    score: clampScore(outcomeScore * 0.6 + fundingScore * 0.4),
    reason: null,
    yearsUsed,
    deploymentYears,

...
export type InflectionInput = {
  ...
  research?: {
    positiveCatalysts?: number;
    negativeCatalysts?: number;
  } | null;
...
export type InflectionSignal = {
  ...
  score: number | null;
  weight: number;
  family: "fundamental" | "expectations" | "market" | "funding" | "research";
  detail: string;
};
...
function average(values: Array<number | null>): number | null {
  const available = values.filter((value): value is number => finite(value));
...
function growthAccelerationScore(current: number | null | undefined, prior: number | null | undefined): number | null {
  ...
  const level = clamp(50 + current * 130);
  const acceleration = clamp(50 + (current - prior) * 220);
  return level * 0.5 + acceleration * 0.5;
}
...
function fundamentalSignal(input: InflectionInput): InflectionSignal {
  ...
  const score = data ? average([
    growthAccelerationScore(data.revenueGrowthCurrent, data.revenueGrowthPrior),
    growthAccelerationScore(data.epsGrowthCurrent, data.epsGrowthPrior),
    improvementScore(data.fcfMarginCurrent, data.fcfMarginPrior, 500),
    improvementScore(data.operatingMarginCurrent, data.operatingMarginPrior, 500),
    improvementScore(data.roicCurrent, data.roicPrior, 500),
  ]) : null;
  return {
    id: "fundamental_acceleration",
    label: "Fundamental acceleration",
    score,
    weight: 0.32,
    family: "fundamental",
    detail: score === null
      ? "Comparable growth, margin or return history is insufficient."
      : "Measures whether growth, cash-flow margins, operating margins and returns are improving rather than merely high in absolute terms.",
  };
}
...
function expectationsSignal(input: InflectionInput): InflectionSignal {
  ...
  return {
    id: "expectations",
    label: "Expectations & revisions",
    score,
    weight: 0.2,
    family: "expectations",
    detail: score === null
      ? "Licensed analyst estimates/revisions are unavailable; missing expectations reduce coverage but are not scored negatively."
...
function overextensionRisk(input: InflectionInput): OverextensionRisk {
  ...
  if (
    (finite(market.oneMonth) && market.oneMonth > 0.3)
    || (finite(market.threeMonth) && market.threeMonth > 0.65)
    || (finite(position) && position > 0.98 && finite(market.threeMonth) && market.threeMonth > 0.4)
  ) return "high";
...
function marketSignal(input: InflectionInput, extension: OverextensionRisk): InflectionSignal {
  ...
  return {
    ...
    family: "market",
    detail: score === null
      ? "Price-trend history is insufficient."
      : extension === "high"
        ? "Trend is strong but parabolic/near-extreme price action is penalized as overextension rather than rewarded blindly."
        : "Uses multiple momentum horizons and 52-week range position for early market confirmation.",
  };
}
...
function fundingSignal(input: InflectionInput): { signal: InflectionSignal; penalty: number; brakes: string[]; critical: boolean } {
  ...
  if (!data) {
    return {
      signal: { id: "funding_quality", label: "Funding & survival quality", score: null, weight: 0.15, family: "funding", detail: "Financial-health evidence is unavailable." },
      penalty: 0,
      brakes: [],
      critical: false,
    };
  ...
  if (finite(data.interestCoverage) && data.interestCoverage > 0 && data.interestCoverage < 2) {
    penalty += 8;
    if (score !== null) score = clamp(score - 25);
    brakes.push("Weak interest coverage increases funding risk.");
  } else if (finite(data.interestCoverage) && data.interestCoverage >= 2 && data.interestCoverage < 4) {
    penalty += 3;
    if (score !== null) score = clamp(score - 10);
    brakes.push("Interest coverage is only moderate.");
  }
  if (data.criticalRisk) {
    penalty += 20;
    score = score === null ? 10 : Math.min(score, 10);
    brakes.push("Critical financial-risk gate is active; strong growth or momentum cannot override survival risk.");
  }
  return {
    signal: {
      id: "funding_quality",
      label: "Funding & survival quality",
      score,
      weight: 0.15,
      family: "funding",
      detail: "Uses StockBox financial-health evidence with explicit dilution and debt-service penalties.",
    },
    penalty,
    brakes,
    critical: Boolean(data.criticalRisk),
  };
}
...
function researchSignal(input: InflectionInput): InflectionSignal {
  ...
  return {
    id: "research_catalysts",
    label: "Research & catalysts",
    score,
    weight: 0.08,
    family: "research",
    detail: score === null
      ? "No verified catalyst evidence is available; StockBox does not invent catalysts."
      : "Uses verified positive and negative catalyst evidence from the research layer.",
  };
}
...
export function computeInflectionAssessment(input: InflectionInput): InflectionAssessment {
  ...
  const signals: InflectionSignal[] = [
    fundamentalSignal(input),
    expectationsSignal(input),
    marketSignal(input, extension),
    funding.signal,
    researchSignal(input),
  ];
  const aggregate = aggregateIntelligenceEvidence(
    signals.map((signal): IntelligenceEvidence => ({
      id: signal.id,
      label: signal.label,
      score: signal.score,
  ...
  const accelerators = signals
    .filter((signal) => finite(signal.score) && signal.score >= 65)
  ...
  return {
    ...
    accelerators,
    brakes,
    overextensionRisk: extension,
    availableFamilies: aggregate.availableFamilies,

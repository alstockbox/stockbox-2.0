...
export type MispricingInput = {
  ...
  trends?: {
    ...
    cashConversion?: number | null; debtToEquity?: number | null; interestCoverage?: number | null; shareGrowth?: number | null;
  };
...
export type MispricingAssessment = {
  score: number | null; confidence: number; coverage: number; label: MispricingLabel; pillars: MispricingPillar[];
  valueTrapRisk: ValueTrapRisk; positiveEvidence: string[]; counterEvidence: string[]; dataAsOf: string | null;
};
...
function priceVsFairValueScore(input: MispricingInput): { score: number | null; detail: string } {
  ...
  return { score: 50 + (directional - 50) * confidence, detail: `DCF base value implies ${(upside * 100).toFixed(1)}% upside/downside versus the current price; DCF confidence ${Math.round(dcf.confidence)}%.` };
}
...
function historicalSelfValuationScore(input: MispricingInput): { score: number | null; detail: string } {
  ...
  return { score: clamp(50 + relativeDiscount * 85, 0, 100), detail: `Current P/E ${history.current.toFixed(1)}x versus historical median ${history.median.toFixed(1)}x across ${history.observationCount} observations.` };
}
...
function deteriorationEvidence(input: MispricingInput): { points: number; reasons: string[] } {
  ...
  const worseningGrowth = (current: number | null | undefined, prior: number | null | undefined, name: string) => {
    ...
    if (current < 0 && prior >= 0) { points += 8; reasons.push(`${name} has moved from positive to negative growth.`); }
    else if (current - prior <= -0.12) { points += 6; reasons.push(`${name} growth has decelerated materially.`); }
  };
  ...
  const marginDeterioration = (current: number | null | undefined, prior: number | null | undefined, name: string) => {
    if (finite(current) && finite(prior) && prior - current >= 0.05) { points += 7; reasons.push(`${name} has contracted by at least five percentage points.`); }
  };
  ...
  if (finite(trends.cashConversion) && trends.cashConversion < 0.5) { points += 6; reasons.push("Cash conversion is weak relative to reported earnings."); }
  if (finite(trends.debtToEquity) && trends.debtToEquity > 2) { points += 7; reasons.push("Leverage is high relative to equity."); }
  if (finite(trends.interestCoverage) && trends.interestCoverage > 0 && trends.interestCoverage < 2) { points += 7; reasons.push("Interest coverage is weak."); }
  if (finite(trends.shareGrowth) && trends.shareGrowth > 0.08) { points += 8; reasons.push("Material share-count growth indicates dilution risk."); }
  if (finite(input.revisionNetLastMonth) && input.revisionNetLastMonth < 0) { points += 6; reasons.push("Analyst estimate revisions are net negative over the last month."); }
  for (const flag of input.redFlags ?? []) {
    if (flag.severity === "critical") { points += 15; reasons.push(`Critical risk flag: ${flag.title}.`); }
    else if (flag.severity === "high") { points += 6; reasons.push(`High-severity risk flag: ${flag.title}.`); }
  }
  return { points: Math.min(45, points), reasons };
}

function valueTrapRiskFor(points: number): ValueTrapRisk { return points >= 20 ? "high" : points >= 8 ? "medium" : "low"; }
...
export function computeMispricingAssessment(input: MispricingInput): MispricingAssessment {
  ...
  const uncertaintyPenalty = (input.sourceConflictSeverity === "high" ? 20 : input.sourceConflictSeverity === "medium" ? 8 : 0) + (input.dataStatus === "stale" ? 15 : input.dataStatus === "unavailable" ? 35 : 0);
  const confidence = intelligenceConfidence(95, aggregate.coverage, uncertaintyPenalty);
  const positiveEvidence = pillars.filter((pillar) => finite(pillar.score) && pillar.score >= 65).sort((a, b) => (b.score as number) - (a.score as number)).map((pillar) => `${pillar.label}: ${pillar.detail}`);
  const counterEvidence = [
    ...trap.reasons,
    ...pillars.filter((pillar) => finite(pillar.score) && pillar.score <= 35).map((pillar) => `${pillar.label}: ${pillar.detail}`),
  ];
  ...
  return { score, confidence, coverage: aggregate.coverage, label: labelFor(score), pillars, valueTrapRisk: valueTrapRiskFor(trap.points), positiveEvidence, counterEvidence, dataAsOf: input.dataAsOf ?? null };
}

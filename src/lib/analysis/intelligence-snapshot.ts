...
function latestFinancialPair(report: AnalysisReport) {
  ...
  return { previous: financials.length >= 2 ? financials.at(-2) ?? null : null, current: financials.at(-1) ?? null };
}
function averageScenarioConfidence(report: AnalysisReport): number {
  const values = (report.engine?.dcf.scenarios ?? []).map((scenario) => scenario.confidence).filter((value): value is number => finite(value));
...
export function mispricingInputFromReport(report: AnalysisReport): MispricingInput {
  ...
  return {
    ...
    trends: {
      ...
      epsGrowthCurrent: current?.epsGrowth ?? report.metrics.epsGrowth1y, epsGrowthPrior: previous?.epsGrowth ?? null,
      fcfMarginCurrent: current?.freeCashFlowMargin ?? report.metrics.fcfMargin, fcfMarginPrior: previous?.freeCashFlowMargin ?? null,
...
export function inflectionInputFromReport(report: AnalysisReport): InflectionInput {
  ...
  return {
    ...
    market: report.market ? {
      ...
      price: report.market.price, yearHigh: report.market.yearHigh, yearLow: report.market.yearLow,
    } : (finite(report.metrics.priceMomentum3m) || finite(report.metrics.priceMomentum1y)) ? {
      oneMonth: null, threeMonth: report.metrics.priceMomentum3m, sixMonth: null, oneYear: report.metrics.priceMomentum1y, price: null, yearHigh: null, yearLow: null,
    } : null,
    funding: { financialHealthScore: dimensionScore(report, "financialHealth"), shareGrowth: current?.shareGrowth ?? null, interestCoverage: current?.interestCoverage ?? report.metrics.interestCoverage, criticalRisk: report.redFlags.some((flag) => flag.severity === "critical") },
    research: hasResearchCatalysts ? { positiveCatalysts: researchPositiveCount, negativeCatalysts: researchNegativeCount } : null,
    dataAsOf: snapshotDataAsOf(report),
  };
}

export function buildIntelligenceSnapshot(report: AnalysisReport, profile: InvestmentProfile = report.investmentProfile): IntelligenceSnapshot {
  ...
  const lensCoreScore = finite(lensReport.score.personalizedScore) ? lensReport.score.personalizedScore : canonicalCoreScore;
  const opportunity = computeOpportunityAssessment({ coreScore: lensCoreScore, mispricingScore: mispricing.score, inflectionScore: inflection.score, profile });
  return { canonicalCoreScore, lensCoreScore, mispricing, inflection, opportunity };
}

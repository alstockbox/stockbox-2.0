...
const DIRECTIONAL_RATINGS = new Set(["Strong Buy", "Buy", "Hold", "Sell", "Strong Sell"]);
...
function scoreRatingMismatch(record: CoverageAuditRecord): boolean {
  const directional = Boolean(record.rating && DIRECTIONAL_RATINGS.has(record.rating));
  if (record.rating === "No Rating") return record.score !== null;
  if (directional) return typeof record.score !== "number" || !Number.isFinite(record.score);
...
export function evaluateCoverageSlo(
  ...
  const discovered = records.filter((record) => !["input_invalid", "not_found", "no_exact_match"].includes(record.status));
  ...
  const supported = records.filter((record) => !["input_invalid", "not_found", "no_exact_match", "unsupported_security_type"].includes(record.status));
  ...
  const directional = records.filter((record) => Boolean(record.rating && DIRECTIONAL_RATINGS.has(record.rating)));
  const directionalWithHighConflict = directional.filter(unresolvedHighConflict);
  ...
  const metrics: CoverageSloMetrics = {
    ...
    directionalHighConflictRate: rate(directionalWithHighConflict.length, directional.length),
  ...
  if (metrics.directionalHighConflictRate > thresholds.maximumDirectionalHighConflictRate) violations.push(`Directional high-conflict rate ${metrics.directionalHighConflictRate.toFixed(4)} exceeds ${thresholds.maximumDirectionalHighConflictRate.toFixed(4)}.`);

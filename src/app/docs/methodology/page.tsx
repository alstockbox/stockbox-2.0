...
export default async function MethodologyPage() {
  ...
  return <>
    ...
    <Section><Container className="max-w-5xl">
      ...
      <p className="mt-8 border-t border-white/10 pt-6 text-xs leading-6 text-[#9aa7b8]">Static benchmarks are versioned fallbacks, not live peer comparisons. Research views are model-based research outputs, not guaranteed outcomes or individualized financial advice. <Link href="/research-standard" className="text-[#e1cb95] hover:text-white">{sv ? "Läs Research Standard" : "Read the Research Standard"}</Link>. <Link href="/data-sources" className="ml-2 text-[#e1cb95] hover:text-white">{sv ? "Läs om datakällorna" : "Read about data sources"}</Link>. <Link href="/nyckeltal" className="ml-2 text-[#e1cb95] hover:text-white">{sv ? "Läs nyckeltalsguider" : "Read metric guides"}</Link>.</p>
    </Container></Section>
  </>;
}

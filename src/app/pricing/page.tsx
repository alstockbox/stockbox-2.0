...
import { isStripeConfigured, isSupabaseConfigured } from "@/lib/env/server";

export const metadata: Metadata = { title: "Pricing", description: "StockBox subscription plans and research limits." };

export default function PricingPage() {
  const checkoutEnabled = isStripeConfigured() && isSupabaseConfigured();
  return (
    <Section>
      <Container>
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          {plans.map((plan) => (
            <article key={plan.key} className={`rounded-lg border p-5 ${plan.highlight ? "border-[#b99b5f]/70 bg-[#b99b5f]/10" : "border-white/10 bg-[#0d1c2e]/75"}`}>
              ...
              <p className="number mt-5 text-3xl font-semibold text-[#f4efe5]">{plan.monthlyPriceSek} kr</p>
              <p className="text-xs text-[#9aa7b8]">per month, VAT handling configured in Stripe</p>
